...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
		...
		case 'anchor' :
			...
			if ( sAnchor.length == 0 )
			{
				alert( TDLang.DlnLnkMsgNoAnchor ) ;
				return false ;
			}
	...
	var aLinks = oLink ? [ oLink ] : oEditor.TD.CreateLink( sUri, true ) ;
	...
	if ( !aHasSelection )
	{
		...
		aLinks = [ oEditor.TD.InsertElement( 'a' ) ] ;
	}
	...
	for ( var i = 0 ; i < aLinks.length ; i++ )
	{
		...
		oLink.href = sUri ;
		SetAttribute( oLink, '_savedurl', sUri ) ;
		...
		if( GetE('cmbTarget').value == 'popup' )
		{
			...
			SetAttribute( oLink, 'onclick_protectedatt', onclick ) ;
		}
		else
		{
			// Check if the previous onclick was for a popup:
			// In that case remove the onclick handler.
			onclick = oLink.getAttribute( 'onclick_protectedatt' ) ;
			if ( onclick )
			{
				// Decode the protected string
				onclick = decodeURIComponent( onclick ) ;
				...
				if( oRegex.OnClickPopup.test( onclick ) )
					SetAttribute( oLink, 'onclick_protectedatt', '' ) ;
			}
		}
	...
	oEditor.TDSelection.SelectNode( aLinks[0] );
...
{
	OpenFileBrowser( TDConfig.LinkBrowserURL, TDConfig.LinkBrowserWindowWidth, TDConfig.LinkBrowserWindowHeight ) ;
}
...
var oUploadAllowedExtRegex	= new RegExp( TDConfig.LinkUploadAllowedExtensions, 'i' ) ;
var oUploadDeniedExtRegex	= new RegExp( TDConfig.LinkUploadDeniedExtensions, 'i' ) ;
...
function CheckUpload()
{
	...
	if ( ( TDConfig.LinkUploadAllowedExtensions.length > 0 && !oUploadAllowedExtRegex.test( sFile ) ) ||
		( TDConfig.LinkUploadDeniedExtensions.length > 0 && oUploadDeniedExtRegex.test( sFile ) ) )
	{
...
{
	var target = TDConfig.DefaultLinkTarget || '' ;
